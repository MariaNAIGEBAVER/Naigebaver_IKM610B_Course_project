...
namespace Naigebaver_IKM610B_Course_project
{
    public partial class Form1 : Form
    {
        ...
        {
            tClock.Stop();
            MessageBox.Show("Минуло 25 секунд", "Увага");// Виведення повідомлення "Минуло 25секунд" на екран
            tClock.Start();
        ...
        private void bStart_Click(object sender, EventArgs e)
        {
            ...
            {
                tbInput.Enabled = false;// Режим заборони введення
                tClock.Stop(); bStart.Text = "Пуск";// зміна тексту на кнопці на "Пуск"
                this.Mode = true;
...
string disk = "";
            for (int i = 0; i < Disks.Length; i++)
            {
                try
                {
                    ...
                    disk += D.Name + "-" + (D.TotalSize/(1024*1024*1024)).ToString() + "-" + (D.TotalFreeSpace/(1024*1024*1024)).ToString()
